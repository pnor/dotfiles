...
Turn off evl snipe mode
#+BEGIN_SRC emacs-lisp
(defun personal-turn-off-evil-snipe-mode ()
  (evil-snipe-mode -1))

(add-hook! 'server-after-make-frame-hook #'personal-turn-off-evil-snipe-mode)
#+END_SRC
...
#+END_SRC
** Edraw
Edraw provides a graphic editor inline in org mode
#+BEGIN_SRC emacs-lisp
...
Return result as a human readable string (thanks ts.el)
Does this by figuring when crash log was last touched."
  (require 'ts)
  (let* ((dir-name (expand-file-name crash-log-dir))
         (last-crash-log (shell-command-to-string (format "ls -r %s | rg emacs | head -1"
                                                          dir-name)))
    ...
    (ts-human-format-duration (ts-diff now-time last-crash-time))))

(defun doom-last-crash-line ()
  "Say how long since Emacs last crashed"
  (format "%s since last incident!" (personal-last-crash-delta "~/Library/Logs/DiagnosticReports")))
