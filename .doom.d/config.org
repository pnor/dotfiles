...
#+begin_src emacs-lisp
(setq doom-font (font-spec :family "CozetteVector" :size 26))
#+end_src
...
(defvar personal-minimap-exclude-modes '(pdf-view-mode vter-mode)
...
(cl-flet ((has-excluded-name (w)
        ...
        (has-excluded-mode (w)
         ;; don't show if one of these modes is included
         (member (with-current-buffer (window-buffer w) major-mode) personal-minimap-exclude-modes)))
    (-any? (lambda (w) (has-excluded-mode w)) (window-list)))
...
(defun personal-manage-minimap-with-splits (num-splits)
  ...
  (let ((window-list-without-minimap
    ...
    (if (personal-should-show-minimap)
        (if (length> window-list-without-minimap (- num-splits 1))
            (demap-close)
          (demap-open))
      (demap-close))))
